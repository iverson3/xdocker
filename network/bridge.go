...
func (b *BridgeNetworkDriver) Create(subnet string, name string) (*NetWork, error) {
	...
	fmt.Println("BridgeNetworkDriver creat network subnet: , gateway ip: ", ipRange.String(), ip.String())
...
func (b *BridgeNetworkDriver) initBridge(n *NetWork) error {
	...
	fmt.Println("createBridgeInterface success")
	...
	fmt.Println("setInterfaceIp success")

	if err := setInterfaceUp(bridgeName); err != nil {
		return err
	}
	fmt.Println("crsetInterfaceUp success")

	if err := setupIpTables(bridgeName, n.IpRange); err != nil {
		return err
	}
	fmt.Println("setInterfaceUp success")
	return nil
}
...
func setInterfaceIp(name string, rawIp string) error {
	...
	for i := 0; i < retries; i++ {
		...
		fmt.Println("error retrieving new bridge netlink link ... retrying", name)
